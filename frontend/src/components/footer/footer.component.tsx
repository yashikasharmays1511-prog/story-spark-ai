...
const FooterComponent: React.FC = () => {
  ...
  return (
    <footer className="relative w-full bg-gradient-to-b from-[#090F24] via-[#080E22] to-[#060A18] overflow-hidden">
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0" style={{ height: "380px", background: "radial-gradient(ellipse 75% 60% at 50% 0%, rgba(56,108,220,0.22) 0%, rgba(79,70,229,0.1) 45%, transparent 80%)" }} />
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0" style={{ height: "240px", background: "radial-gradient(ellipse 50% 40% at 50% -5%, rgba(99,130,255,0.13) 0%, rgba(79,70,229,0.05) 50%, transparent 80%)" }} />
      <div aria-hidden="true" className="pointer-events-none absolute right-[10%] top-[15%] w-[320px] h-[320px]" style={{ background: "radial-gradient(circle, rgba(56,108,220,0.08) 0%, rgba(79,70,229,0.03) 50%, transparent 75%)", filter: "blur(40px)" }} />
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 h-px" style={{ background: "linear-gradient(90deg, transparent 0%, rgba(99,130,255,0.35) 35%, rgba(139,92,246,0.20) 65%, transparent 100%)" }} />

      <div className="relative z-10 max-w-[1450px] mx-auto px-6 sm:px-8 lg:px-10 pt-12 pb-6 sm:pb-8">
        <div className="grid grid-cols-12 gap-x-6 gap-y-10 items-start">

          <div className="col-span-12 lg:col-span-4 flex flex-col gap-5">
            <Link to="/" className="group inline-block w-fit">
              <img src={logo} alt="StorySparkAI" className="h-[38px] w-auto object-contain brightness-100 transition-all duration-300 group-hover:brightness-110" />
            </Link>
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-blue-500/20 bg-blue-600/[0.10] px-3.5 py-1 text-[11.5px] font-medium tracking-[0.12em] text-blue-300/90 uppercase">
          ...
          <div className="col-span-6 sm:col-span-4 lg:col-span-2 flex flex-col gap-4">
            <h3 className="text-[11.5px] font-bold tracking-[0.22em] uppercase text-white/70">Follow Us</h3>
            <ul className="flex flex-col gap-[12.5px]">
              {socialLinks.map((item) => (
                <li key={item.icon}>
                  <a href={item.url} target="_blank" rel="noopener noreferrer" aria-label={item.label} className="group flex items-center gap-2.5 text-[14px] text-slate-300/85 hover:text-blue-300 transition-all duration-200">
                    {item.icon === "fa-x-twitter" ? (
                      <FaXTwitter className="text-[15px] text-slate-400 group-hover:text-blue-300 transition-colors" />
                    ) : (
                      <i className={`fa-brands ${item.icon} text-[15px] text-slate-400 group-hover:text-blue-300 transition-colors`} />
                    )}
                    <span className="capitalize">
                      {item.icon === "fa-x-twitter" ? "X (Twitter)" : item.icon.replace("fa-", "")}
                    </span>
                  </a>
                </li>
              ))}
          ...
          <div className="col-span-12 sm:col-span-8 lg:col-span-2 flex flex-col gap-4">
            ...
            <form onSubmit={handleSubscribe} noValidate className="mt-1 flex flex-col gap-2">
        ...
        <div className="my-6 sm:my-8" style={{ height: "1px", background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.07) 30%, rgba(255,255,255,0.07) 70%, transparent 100%)" }} />

        <div className="flex flex-col md:flex-row items-center justify-between gap-3 text-[12px] text-slate-400/80">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-2.5 gap-y-1">
            {legalLinks.map(({ label, to }, i) => (
              <React.Fragment key={label}>
                <Link to={to} className="hover:text-blue-300 transition-colors">{label}</Link>
                {i < legalLinks.length - 1 && <span className="text-white/12">|</span>}
              </React.Fragment>
            ))}
          </div>
        </div>
