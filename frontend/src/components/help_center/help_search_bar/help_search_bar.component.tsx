    ...
    >
      ...
      <div className="relative before:absolute before:inset-0 before:-z-10 before:bg-gradient-to-r before:from-purple-500/10 before:via-indigo-500/10 before:to-blue-500/10 before:blur-xl before:rounded-2xl">
        ...
        <input
          ...
          className="w-full bg-white text-slate-800 placeholder-slate-400 border border-slate-300 shadow-sm dark:bg-white/5 dark:backdrop-blur-sm dark:border-white/20 dark:text-gray-200 dark:placeholder-gray-500 rounded-2xl py-3.5 sm:py-4 pl-11 sm:pl-12 pr-11 sm:pr-12 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all duration-300 box-border appearance-none [&::-webkit-search-cancel-button]:hidden"
          autoComplete="off"
        />
      ...
      {value && resultCount !== undefined && (
        <p
          className="mt-3 text-xs sm:text-sm text-slate-500 dark:text-gray-500 text-center font-medium tracking-wide"
          aria-live="polite"
        >
          {resultCount === 0
            ...
            : `${resultCount} result${
                resultCount === 1 ? "" : "s"
              } uncovered inside ecosystem guides`}
        </p>
      )}
    </form>
  );
